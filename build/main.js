...
class Faut extends utils.Adapter {
     ...
     */
    collectAlexaMappings(nodes, prefix, parentRoomRelId) {
        for (const node of nodes) {
            const relId = prefix ? `${prefix}.${node.label}` : node.label;
            const cfg = node.config ?? {};
    ...
    collectPersonMappings(nodes, prefix) {
        for (const node of nodes) {
            const relId = prefix ? `${prefix}.${node.label}` : node.label;
            const cfg = node.config ?? {};
            ...
            if (node.children?.length)
                this.collectPersonMappings(node.children, relId);
        }
    }
    ...
    postMessage(source, severity, message, needAck = false, msgTimeout = MSG_DEFAULT_TIMEOUT_S) {
        ...
        this.sendAlexaMessage(msg).catch(e => this.log.warn(`Alexa send failed: ${e.message}`));
    }
    /**
     ...
     */
    async isAnyoneHome() {
        if (this.personResidentDps.length === 0)
            return false;
    ...
    async sendAlexaMessage(msg) {
        ...
        if (targetDps.length === 0) {
            const globalGroup = this.config.alexaMultiroomGruppe ?? '';
            if (globalGroup)
                targetDps.push(globalGroup);
        }
        if (targetDps.length === 0)
            return;
        ...
        for (const dp of targetDps) {
            try {
                ...
                this.log.warn(`Alexa speak failed for ${dp}: ${e.message}`);
            }
        }
    }
    /**
