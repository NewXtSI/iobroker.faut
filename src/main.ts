...
/** A notification message managed by the adapter. */
interface FautMessage {
	uuid:       string;
	severity:   'info' | 'warning' | 'error';
	message:    string;
	/** Internal key used for deduplication – postMessage with same source replaces the old one. */
	source:     string;
	needAck:    boolean;
	/** Seconds until auto-expiry. 0 = no expiry. For needAck=true: starts after ack. */
	msgTimeout: number;
	createdAt:  number;
	acked:      boolean;
	ackedAt?:   number;
...
class Faut extends utils.Adapter {
	...
	private readonly dpToBoolUnreachMap = new Map<string, { unreachRelId: string; isAlive: boolean }>();
	/** Tracks the current unreach boolean per unreach-state relId (for change detection). */
	private readonly unreachValues   = new Map<string, boolean>();
	...
	private messagesCheckTimer: ReturnType<typeof setInterval> | null = null;
	...
	private async onReady(): Promise<void> {
		...
		const systemLang = (sysObj?.common as any)?.language as string | undefined;
	...
	private async setupGlobalStates(): Promise<void> {
		...
		try {
			...
			if (typeof msgSt?.val === 'string' && msgSt.val !== '[]') {
				this.messages = JSON.parse(msgSt.val) as FautMessage[];
			}
		} catch (e) {
			this.log.warn(`Failed to load persisted messages: ${(e as Error).message}`);
		}
	...
	private async setupBatteryAndUnreach(): Promise<void> {
		...
		for (const [dpId, { unreachRelId, isAlive }] of this.dpToBoolUnreachMap) {
			...
			} catch (e) { this.log.warn(`Initial bool-unreach read failed for ${dpId}: ${(e as Error).message}`); }
		}
	...
	private getSensorStateSpecs(
		...
		if (nodeType === 'Temperatur') {
			...
			if (cfg.dpThermostatTemperatur)     specs.push({ id: 'temperature', name: 'Temperature', dataType: 'number', role: 'value.temperature', unit: '\u00b0C' });
			if (cfg.dpThermostatSolltemperatur) specs.push({ id: 'setpoint',     name: 'Setpoint',     dataType: 'number', role: 'level.temperature', unit: '\u00b0C', write: true });
	 ...
	 */
	removeMessage(source: string): void {
	...
	checkMessageTimeouts(): void {
		const now    = Date.now();
		const before = this.messages.length;
		this.messages = this.messages.filter(msg => {
			if (msg.msgTimeout <= 0) return true;
			if (!msg.needAck) {
	...
	private async sendTelegramMessage(msg: FautMessage): Promise<void> {
		const instanz = (this.config.telegramInstanz as string | undefined) ?? '';
		if (!instanz) return;
	...
	private onUnload(callback: () => void): void {
		try {
			...
			if (this.messagesCheckTimer !== null) clearInterval(this.messagesCheckTimer);
			for (const room of this.shutterRooms.values()) {
	...
	private onStateChange(id: string, state: ioBroker.State | null | undefined): void {
		...
		if (this.dpToBoolUnreachMap.has(id)) {
			...
			if (dpVal !== null) {
				...
				if (isUnreach !== wasUnreach) {
					this.unreachValues.set(unreachRelId, isUnreach);
					this.setStateAsync(unreachRelId, { val: isUnreach, ack: true }).catch(e =>
						this.log.error(`Bool-unreach set failed for ${unreachRelId}: ${(e as Error).message}`));
					const baseRelId = unreachRelId.endsWith('.unreach') ? unreachRelId.slice(0, -8) : unreachRelId;
					const label = this.labelFor(baseRelId);
					if (isUnreach) {
						this.postMessage(`unreach.${unreachRelId}`, 'warning', `${label}: ${i18n.t('Unreachable')}`, true, 0);
		...
		if (id === `${this.namespace}.global.messages` && !state.ack && typeof state.val === 'string') {
			try {
				const incoming = JSON.parse(state.val) as FautMessage[];
				...
				this.saveMessages().catch(e => this.log.error(`saveMessages (VIS ack) failed: ${(e as Error).message}`));
			} catch (e) {
				this.log.warn(`global.messages external write parse error: ${(e as Error).message}`);
			}
			return;
		}
